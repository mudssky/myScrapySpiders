...
class getchuIDSpider(scrapy.Spider):
                ...
                if type == 'other':
                    target_ids = {
                        ...
                        for x in self.collection.find(
                            {'tab_type': 'unknown', 'res_body_size': {'$gt': 2000}}
                        )
                    }
                    ...
                    target_ids = {
                        x['getchu_id']
                        for x in self.collection.find({'tab_type': 'game'})
                    }
                    yield from (id for id in input_ids if id in target_ids)
                elif type == 'unknown':
                    target_ids = {
                        x['getchu_id']
                        for x in self.collection.find({'tab_type': 'unknown'})
                    }
                    yield from (id for id in input_ids if id in target_ids)
                elif type == 'not_in_db':
                    # 过滤不在数据库中的id
    ...
    def extract_common(self, response, loader=None):
        ...
        common_dict['media'] = soft_table.xpath(
    ...
    def parse(self, response):
        ...
        if tab_type == 'game' or tab_type == 'dvdpg' or tab_type == 'adult_goods':
            ...
            # l.add_value('media', common_dict['media'])
            ...
            if tab_type == 'music':
                ...
                l = getchu_item.GameItemLoader(
                    item=getchu_item.GetchuItem(), response=response
                )
            self.extract_common(response=response, loader=l)
            return l.load_item()
            ...
            l = getchu_item.UnknownItemLoader(
